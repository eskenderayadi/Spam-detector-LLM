...
A [Chainlit](https://github.com/Chainlit/chainlit) app that classifies text messages as `spam` or `not spam` using OpenAI’s GPT-2 and [**LoRA**](https://github.com/microsoft/LoRA) (Low-Rank Adaptation), achieving similar results with 9M trainable parameters instead of the full 124M base model.

The model architecture and training workflow are inspired by Sebastian Raschka’s [book](https://www.manning.com/books/build-a-large-language-model-from-scratch) and its [reference implementation](https://github.com/rasbt/LLMs-from-scratch). See [Citation](#citation).
